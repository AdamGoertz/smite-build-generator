...
class BuildTracker:
    ...
    def get(self, item: Item) -> ItemTracker:
        return self.trackers.get(item, ItemTracker(item))
    ...
    def items(self) -> Collection[Item]:
        ...
        return sorted(self.trackers.keys(), key=lambda item: self.trackers.get(item, ItemTracker(item)).count, reverse=True)
